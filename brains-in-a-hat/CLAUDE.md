...
- **Permission requests from teammates** — two types: (1) **Tool perm requests**: CC surfaces these directly to Neal as interrupts (no hook). Neal approves normal tool use and denies clearly destructive patterns. (2) **Plan approval requests**: teammate in plan mode sends a message to Neal when planning is done; Neal approves or rejects via SendMessage.
