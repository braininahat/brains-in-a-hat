...
- **Permission requests from teammates** — two types: (1) **Tool perm requests**: teammates run with `mode="bypassPermissions"` so they never generate tool permission requests (each teammate is a headless session with no terminal to surface them to). (2) **Plan approval requests**: teammate in plan mode sends a message to Neal when planning is done; Neal approves or rejects via SendMessage.
