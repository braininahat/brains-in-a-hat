...
After writing retrospectives to `.claude/team/retrospectives/`, also write an Obsidian-native version to the vault:

- Retrospective → `~/.claude/vault/projects/<project>/retros/YYYY-MM-DD.md` (use `vault-templates/retro.md`)
- Include `[[wikilinks]]` to related decisions and `[[agent-<name>]]` references
- Tag with `#retro` and relevant `#agent/*` tags
- Include Dataview frontmatter: `type: retro`, `project`, `agents`, `date`, `tags`, `status: active`
...
```bash
echo '{"ts":"'$(date -Iseconds)'","agent":"meta-retro","event":"<TYPE>","detail":"<TEXT>"}' >> .claude/team/activity.jsonl
```
